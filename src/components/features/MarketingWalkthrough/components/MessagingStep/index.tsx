...
const MessageFramework: React.FC<MessageFrameworkProps> = ({ onSave, formData, setFormData }) => {
  ...
  // Export Message House with visual design
  const exportMessageHouse = async (format: 'txt' | 'pdf' | 'docx' | 'visual-pdf' = 'txt') => {
    if (!hasContent()) {
    ...
    // Visual PDF export (captures the styled design)
    if (format === 'visual-pdf') {
      const messageHouseContent = document.querySelector('.bg-gradient-to-br.from-blue-50.to-indigo-50');
      if (!messageHouseContent) {
        showNotification('Could not find Message House content to export', 'error');
        return;
      }
      ...
      try {
        ...
        const canvas = await html2canvas(messageHouseContent as HTMLElement, {
          scale: 2, // Higher quality
          useCORS: true,
          allowTaint: false,
          backgroundColor: null,
          width: (messageHouseContent as HTMLElement).scrollWidth,
          height: (messageHouseContent as HTMLElement).scrollHeight,
          scrollX: 0,
          scrollY: 0
        });
        ...
        showNotification('Visual Message House PDF exported successfully', 'success');
        setShowMessageHouseExport(false);
        return;
      } catch (error) {
        console.error('❌ Error creating visual PDF:', error);
        showNotification('Failed to create visual PDF. Try the text version instead.', 'error');
        return;
      }
    }

    // Rest of your existing text export code stays the same...
    const content = `
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 border-t bg-gray-50">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                <button
                  onClick={() => exportMessageHouse('visual-pdf')}
                  className="p-4 text-left border-2 border-blue-500 bg-blue-50 rounded-md hover:bg-blue-100 transition-all"
                >
                  <div className="flex items-center gap-3">
                    <FileText className="w-5 h-5 text-blue-600" />
                    <div>
                      <div className="font-medium text-blue-700">Visual PDF</div>
                      <div className="text-sm text-blue-600">Styled design</div>
                    </div>
                  </div>
                </button>
                <button

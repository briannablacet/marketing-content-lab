...
const ContentEditChat = ({
  ...
  const getPromptSuggestions = () =>
    PROMPT_SUGGESTIONS[contentType] || PROMPT_SUGGESTIONS["default"];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log(
        "🚀 SUBMITTING with payload:",
        JSON.stringify(payload).slice(0, 500)
      );
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/documents/modify`,
        {
          method: "POST",
          headers: {
      ...
      if (!result?.processedContent) {
        throw new Error("Processed content not found in response.");
      }
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: result.processedContent },
      ]);

      if (onContentUpdate) {
        onContentUpdate(result.processedContent, originalTitle);
      }
      setStatusMessage({
        type: "success",
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: "Sorry, I was unable to update the content.",
        },
      ]);
        ...
        {messages.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p className="text-sm mt-2">
              For example, "Make it more persuasive" or "Add statistics"
            </p>
          </div>
        ) : (
      ...
      {(strategicContext || loadedStrategicData) && (
        <div className="px-4 py-2 bg-green-50 text-green-800 text-xs border-t border-green-100">
          <div className="flex items-center">
            ...
            Using your marketing program's strategic data for content
            improvements
          </div>
        </div>
      )}
        ...
        <div className="flex overflow-x-auto gap-2 pb-2">
          {getPromptSuggestions().map((suggestion, index) => (
            <button
              ...
              className={`px-3 py-1.5 border rounded-full text-sm whitespace-nowrap ${
